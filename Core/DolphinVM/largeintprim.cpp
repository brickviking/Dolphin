...
unsigned highBit(uint32_t mask)
{
	unsigned long index;
...
Oop* __fastcall Interpreter::primitiveLargeIntegerAdd(Oop* const sp, unsigned)
{
	...
	struct liAddSingle
	{
		Oop __forceinline operator()(const LargeIntegerOTE* oteLI, SMALLINTEGER operand) const
		{
			ASSERT(operand != 0);
			__assume(operand != 0);

			const uint32_t* digits = oteLI->m_location->m_digits;
			...
			if (negative != static_cast<int32_t>(accum) < 0)
			{
				...
				sum = reinterpret_cast<LargeInteger*>(ObjectMemory::resize(reinterpret_cast<BytesOTE*>(oteSum), (limbs + 1) << 2))->m_digits;
				sum[limbs] = negative * -1;
			}
	...
	struct liAdd
	{
		Oop __forceinline operator()(const LargeIntegerOTE* oteOp1, const LargeIntegerOTE* oteOp2) const
		{
			MWORD size1 = oteOp1->getWordSize();
			MWORD size2 = oteOp2->getWordSize();

			uint32_t *digits1, *digits2;

			if (size1 < size2)
			{
				if (size1 == 1)
				{
					return liAddSingle()(oteOp2, static_cast<int32_t>(oteOp1->m_location->m_digits[0]));
				}
			...
			else
			{
				if (size2 == 1)
				{
					return liAddSingle()(oteOp1, static_cast<int32_t>(oteOp2->m_location->m_digits[0]));
				}
			...
			if (size1 > size2)
			{
				...
				int64_t accum = static_cast<int64_t>(digits1[i]) + static_cast<int32_t>(digits2[i]) + carry;
				sum[i++] = LowLimb(accum);
				carry = HighSLimb(accum);
				...
				for (; i < size1 - 1; i++)
				{
					int64_t accum = static_cast<int64_t>(digits1[i]) + carry;
					sum[i] = LowLimb(accum);
					carry = HighSLimb(accum);
				}
				...
				accum = static_cast<int64_t>(static_cast<int32_t>(digits1[i])) + carry;
				sum[i] = LowLimb(accum);
				carry = HighSLimb(accum);
			}
			else
			{
				ASSERT(i == size1 - 1);
				int64_t accum = static_cast<int64_t>(static_cast<int32_t>(digits1[i])) + static_cast<int32_t>(digits2[i]) + carry;
				sum[i] = LowLimb(accum);
				carry = HighSLimb(accum);
			}
			...
			if ((carry < 0) != (static_cast<int32_t>(sum[i]) < 0))
			{
				// Add extra digit necessary to represent the sign
	...
	return primitiveLargeIntegerOpR(sp, liAdd(), liAddSingle());
}
...
Oop* __fastcall Interpreter::primitiveLargeIntegerSub(Oop* const sp, unsigned)
{
	...
	struct liSub
	{
		Oop __forceinline operator()(const LargeIntegerOTE* oteLI, const LargeIntegerOTE* oteOperand) const
		{
	...
	return primitiveLargeIntegerOpR(sp, liSub(), liSubSingle());
}
...
Oop* __fastcall Interpreter::primitiveLargeIntegerMul(Oop* const sp, unsigned)
{
	...
	private:
		...
		Oop mul(const LargeInteger* liOuter, const MWORD outerSize, const LargeInteger* liInner, const MWORD innerSize) const
		{
	...
	public:
		Oop __forceinline operator()(const LargeIntegerOTE* oteOuter, const LargeIntegerOTE* oteInner) const
		{
			...
			return outerSize > innerSize ? mul(liInner, innerSize, liOuter, outerSize) :
				mul(liOuter, outerSize, liInner, innerSize);
		}
	};
	...
	struct liMulSingle
	{
		Oop __forceinline operator()(const LargeIntegerOTE* oteInner, SMALLINTEGER outerDigit) const
		{
			ASSERT(outerDigit != 0);
	...
	return primitiveLargeIntegerOpZ(sp, liMul(), liMulSingle());
}
...
template <class UCmp, class ICmp> __forceinline static Oop* primitiveLargeIntegerCmp(Oop* const sp, const ICmp &icmp, const UCmp& ucmp)
{
	Oop argPointer = *sp;
	...
	if (ObjectMemoryIsIntegerObject(argPointer))
	{
		...
		int sign = static_cast<int32_t>(liReceiver->m_digits[oteReceiver->getWordSize() - 1]) < 0 ? -1 : 1;
		*(sp - 1) = reinterpret_cast<Oop>(icmp(sign, 0) ? Pointers.True : Pointers.False);
		return sp - 1;
	}
	else
	{
		...
		if (oteArg->m_oteClass == Pointers.ClassLargeInteger)
		{
			LargeInteger* liArg = oteArg->m_location;

			const MWORD receiverSize = oteReceiver->getWordSize();
			const MWORD argSize = oteArg->getWordSize();

			const int receiverSign = static_cast<int32_t>(liReceiver->m_digits[receiverSize - 1]) < 0 ? -1 : 1;
			const int argSign = static_cast<int32_t>(liArg->m_digits[argSize - 1]) < 0 ? -1 : 1;

			if (receiverSign != argSign)
			{
				// Different signs
				*(sp - 1) = reinterpret_cast<Oop>(icmp(receiverSign - argSign, 0) ? Pointers.True : Pointers.False);
				return sp - 1;
			}
			else
			{
				// Same sign
				if (receiverSize == argSize)
				{
					// Same size and sign, need to compare limbs
					for (int i = receiverSize - 1; i >= 0; i--)
					{
						const uint32_t digA = liReceiver->m_digits[i];
						const uint32_t digB = liArg->m_digits[i];
						if (digA != digB)
						{
							*(sp - 1) = reinterpret_cast<Oop>(ucmp(digA, digB) ? Pointers.True : Pointers.False);
							return sp - 1;
						}
					}

					// The condition should compile down to a constant here
					*(sp - 1) = reinterpret_cast<Oop>(icmp(0, 0) ? Pointers.True : Pointers.False);
					return sp - 1;
				}

				// Different sizes, can compare based on length
				*(sp - 1) = reinterpret_cast<Oop>(icmp((static_cast<int>(receiverSize) - static_cast<int>(argSize)) * receiverSign, 0) ? Pointers.True : Pointers.False);
				return sp - 1;
			}
		}
	}
...
{
	return primitiveLargeIntegerCmp(sp, std::less<int32_t>(), std::less<uint32_t>());
}

Oop* __fastcall Interpreter::primitiveLargeIntegerGreaterThan(Oop* const sp, unsigned)
{
	return primitiveLargeIntegerCmp(sp, std::greater<int32_t>(), std::greater<uint32_t>());
}

Oop* __fastcall Interpreter::primitiveLargeIntegerLessOrEqual(Oop* const sp, unsigned)
{
	return primitiveLargeIntegerCmp(sp, std::less_equal<int32_t>(), std::less_equal<uint32_t>());
}

Oop* __fastcall Interpreter::primitiveLargeIntegerGreaterOrEqual(Oop* const sp, unsigned)
{
	return primitiveLargeIntegerCmp(sp, std::greater_equal<int32_t>(), std::greater_equal<uint32_t>());
}

Oop* __fastcall Interpreter::primitiveLargeIntegerEqual(Oop* const sp, unsigned)
{
	// We Implement this specially, because the generic comparison template does a lot more work than necessary for equality comparison
	Oop argPointer = *sp;
	...
	else
	{
		...
		if (oteReceiver != oteArg)
		{
			if (oteArg->m_oteClass == Pointers.ClassLargeInteger)
			{
				...
				{
					// If the same size, we can just memcpy the content. Has to be identical to be equal. 
					...
					// Same size, but need same sign and content to be equal
...
Oop* __fastcall Interpreter::primitiveLargeIntegerBitAnd(Oop* const sp, unsigned)
{
	...
	struct liBitAndSingle
	{
		Oop operator()(const LargeIntegerOTE* oteA, SMALLINTEGER mask) const
		{
			...
			else
			{
				...
				// Easier in the sense that only a single digit 
	...
	return primitiveLargeIntegerOpZ(sp, liBitAnd(), liBitAndSingle());
}
...
Oop* __fastcall Interpreter::primitiveLargeIntegerBitOr(Oop* const sp, unsigned)
{
	...
	return primitiveLargeIntegerOpR(sp, liBitOr(), liBitOrSingle());
}
...
Oop* __fastcall Interpreter::primitiveLargeIntegerBitXor(Oop* const sp, unsigned)
{
	...
	return primitiveLargeIntegerOpR(sp, liBitXor(), liBitXorSingle());
}
